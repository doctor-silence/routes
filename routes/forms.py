...
class RouteForm(forms.Form):    # Проверка соблюдения уникальности
...
class RouteModelForm(forms.ModelForm):
    ...
    trains = forms.ModelMultipleChoiceField(label='Через города', queryset=Train.objects.all(), required=False, widget=forms.SelectMultiple(attrs={
        'class': 'form-control d-none'}
    ))
    travel_time = forms.IntegerField(label='Время в пути', widget=forms.HiddenInput())
