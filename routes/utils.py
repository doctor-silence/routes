...
def get_routes(request, form) -> dict:
    ...
    for route in right_ways:
        ...
        if total_time <= travelling_time:
            #Если общее время в пути меньше заданнного, то добавляем маршрут в общий список
            routes.append(tmp)
    if not routes:
        #Если спиок пуст, то неет таких маршрутов которые удовлетворяли бы заданным условиям
